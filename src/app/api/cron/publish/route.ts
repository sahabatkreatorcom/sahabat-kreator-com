...
export async function GET(request: Request) {
  ...
  const scheduledPosts = await db
    .select()
    .from(post)
    .where(and(eq(post.status, "scheduled"), isNull(post.publishedAt)));
  ...
  for (const p of duePosts) {
    try {
      ...
      if (accounts.length === 0) {
        await db.update(post).set({ status: "failed", errorMessage: "Account not found", updatedAt: nowIso }).where(eq(post.id, p.id));
      ...
      await db.update(post).set({ status: "published", publishedAt: nowIso, platformPostId: result.platformPostId, updatedAt: nowIso }).where(eq(post.id, p.id));
      results.push({ id: p.id, status: "published" });
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Unknown error";
      await db.update(post).set({ status: "failed", errorMessage, updatedAt: nowIso }).where(eq(post.id, p.id));
